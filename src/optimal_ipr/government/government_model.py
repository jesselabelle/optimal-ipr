...
class GovernmentModel:
    ...
    def _calculate_welfare_for_v(
        ...
        return {
            ...
            "imit_util": float(imit_util),
            "non_invest_util": float(transfer) * num_non_investors,
    ...
    def solve(self, *, government_scheme: str, regulator_scheme: str):
        ...
        for bar_beta in self.bar_grid:
            sims = [
                self._calculate_welfare_for_v(w, regulator_scheme, float(bar_beta), float(v))
                for v in self.v_grid
            ]
            welfare_arr = np.array([s["welfare_for_v"] if s else 0.0 for s in sims], dtype=float)
            beta_arr = np.array([s["beta_star"] if s else 0.0 for s in sims], dtype=float)
            reg_welfare_arr = np.array(
                [s["reg_welfare_for_v"] if s else 0.0 for s in sims], dtype=float
            )
            innov_util_arr = np.array([s["innov_util"] if s else 0.0 for s in sims], dtype=float)
            non_innov_arr = np.array(
                [(s["imit_util"] + s["non_invest_util"]) if s else 0.0 for s in sims],
                dtype=float,
            )
            results_list.append(
                {
                    "bar_beta": float(bar_beta),
                    "expected_welfare": float(np.dot(self.v_weights, welfare_arr)),
                    "expected_beta_star": float(np.dot(self.v_weights, beta_arr)),
                    "expected_reg_welfare": float(np.dot(self.v_weights, reg_welfare_arr)),
                    "expected_innov_util": float(np.dot(self.v_weights, innov_util_arr)),
                    "expected_non_innov_util": float(np.dot(self.v_weights, non_innov_arr)),
                }
            )
